...
public class ThreadWait extends Thread{
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        Object b = new Object();
        Object c = new Object();
        ThreadWait ta = new ThreadWait("A" , c ,a);
        ThreadWait tb = new ThreadWait("B",a , b);
        ThreadWait tc = new ThreadWait("C",b,c);
        new Thread(ta).start();
        Thread.sleep(100);//确保按照abc执行
        new Thread(tb).start();
        Thread.sleep(100);
        new Thread(tc).start();
        Thread.sleep(100);
     ...
     *  主要的思想就是，为了控制执行的顺序，必须要先持有prev锁，也就前一个线程要释放自身对象锁，再去申请自身对象锁，两者兼备时打印，
     *  之后首先调用self.notify()释放自身对象锁，唤醒下一个等待线程，再调用prev.wait()释放prev对象锁，终止当前线程，
     *  等待循环结束后再次被唤醒。运行上述代码，可以发现三个线程循环打印ABC，共10次。
